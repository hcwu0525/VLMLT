...
# virtual environment
source ~/.bashrc
source ~/anaconda3/bin/activate dino
...
OMP_NUM_THREADS=4 srun --partition=MoE --job-name="infer" --mpi=pmi2  --gres=gpu:${gpus} -n1 --ntasks-per-node=1 -c ${cpus} --kill-on-bad-exit=1 --quotatype=${quotatype}  \
accelerate launch --config_file=${accelerate_config_file}  groundingDINO_inference.py \
--input_path /mnt/petrelfs/songmingyang/songmingyang/data/llava_train/LLaVA-Instruct-150K/new/llavaft_llamaobj.jsonl \
--dataset_type sharegpt4v_infer \
--image_dir_path /mnt/petrelfs/songmingyang/songmingyang/data/mm/imgs \
--output_path /mnt/petrelfs/songmingyang/songmingyang/data/llava_train/LLaVA-Instruct-150K/new/llavaft_dinoobj.jsonl \
--token_path /mnt/petrelfs/songmingyang/songmingyang/data/llava_train/LLaVA-Instruct-150K/new/llavaft_tokens.jsonl \
--batch_size 16 \
