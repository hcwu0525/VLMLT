...
cpus=32
quotatype="auto"
OMP_NUM_THREADS=4 srun --partition=MoE --job-name="control" --mpi=pmi2  --gres=gpu:${gpus} -n1 --ntasks-per-node=1 -c ${cpus} --kill-on-bad-exit=1 --quotatype=${quotatype}  \
...
--output_path /mnt/petrelfs/songmingyang/songmingyang/data/mm/imgs/latent_aug/llavaft_selfaug_control \
