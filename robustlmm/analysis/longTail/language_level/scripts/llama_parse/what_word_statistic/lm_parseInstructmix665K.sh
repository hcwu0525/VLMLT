...
OMP_NUM_THREADS=8 srun --partition=MoE --job-name="LM_INSTRUCT665K" --mpi=pmi2 --gres=gpu:${gpus}  -n1 --ntasks-per-node=1 -c ${cpus} --kill-on-bad-exit=1 --quotatype=${quotatype}  \
...
--output_file_path /mnt/petrelfs/songmingyang/songmingyang/data/llava_train/LLaVA-Instruct-150K/llava_v1_5_mix665k_what_word_llama.jsonl \
--verbosity info \
--gpus ${gpus} \
